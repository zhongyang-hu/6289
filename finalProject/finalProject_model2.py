...
model1.add(LSTM(128, activation='relu',dropout=0.2, return_sequences=True))
...
epochs=100
modelcheck = ModelCheckpoint('model2.h5', monitor='val_sparse_categorical_accuracy', mode='max', verbose=1, save_best_only=True)
history = model1.fit(xtrain, ytrain, epochs=epochs, batch_size=64, validation_data=(xtest, ytest), callbacks=[modelcheck])
trainhist = pd.DataFrame(history.history)
trainhist.to_csv('model2.csv')